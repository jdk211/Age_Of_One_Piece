...
HRESULT progressBar::init( int x, int y, int width, int height )
{
	...
	_progressBarForward = IMAGEMANAGER->addImage( "barForward", "image/hud/waitbar.bmp", x, y, width, height, false, false );
...
{
	_rcProgress = RectMakeCenter( _x, _y, _progressBarForward->getWidth(), _progressBarForward->getHeight() );
}
...
void progressBar::render(int x)
{
	...
	*/
	...
	_rcProgress = RectMakeCenter(x, _y, _progressBarForward->getWidth(), _progressBarForward->getHeight());
...
{
	_width = ( currentGauge / maxGauge ) * _progressBarForward->getWidth();
}
